...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        ...
        if (Math.abs(x_diff)<1e-10) {
            return Double.POSITIVE_INFINITY;
        }        
        if (Math.abs(that.y - this.y)<1e-10) {
            return 0;
        }        
    ...
    public int compareTo(Point that) {
        ...
        double y_diff = that.y - this.y;
        double x_diff = that.x - this.x;
        if ((Math.abs(y_diff)<1e-10) && (Math.abs(x_diff)<1e-10)) {
            return 0;
        } else if ((this.y < that.y) || ((Math.abs(y_diff)<1e-10) && (this.x < that.x))) {
            return -1;
        } else {
            return 1;
        }
